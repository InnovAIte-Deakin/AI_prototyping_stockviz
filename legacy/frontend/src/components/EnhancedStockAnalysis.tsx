...
try {
  ...
  EnhancedStockService =
    require("@/services/enhancedStockService").default ||
    require("@/services/enhancedStockService");
} catch (e) {
  ...
  const handleAnalysisSubmit = async () => {
    ...
    try {
      ...
      if ((EnhancedStockService as any)?.analyzeStock) {
        const res = await (EnhancedStockService as any).analyzeStock({
        ...
        if (res && (res.aiSummary || res.recommendation)) {
          ...
          const rec =
            res.analysis?.overall?.recommendation ||
            res.analysis?.overall ||
            res.recommendation ||
            "HOLD";
          const score =
            res.analysis?.overall?.score ??
            res.recommendation?.score ??
            res.analysis?.enhanced?.aggregateScore ??
            50;
          ...
          if (res.insights) {
            aiCandidates.push(
              res.insights.full_analysis || res.insights.fullAnalysis,
            );
            aiCandidates.push(
              res.insights.executive_summary || res.insights.executiveSummary,
            );
            aiCandidates.push(
              res.insights.investment_recommendation ||
                res.insights.investmentRecommendation,
            );
          }
          ...
          if (res.analysis) {
            ...
            aiCandidates.push(
              ains.full_analysis ||
                ains.fullAnalysis ||
                ains.fullAnalysisText ||
                ains.full_analysis_text,
            );
            aiCandidates.push(ains.executive_summary || ains.executiveSummary);
            aiCandidates.push(
              ains.investment_recommendation ||
                ains.investmentRecommendation ||
                ains.recommendation,
            );
          ...
          const currentPrice =
            res.currentPrice || res.analysis?.meta?.price || "$0.00";

          setAnalysisResult({
            companyName,
            symbol,
            recommendation: {
              action: typeof rec === "string" ? rec : rec.action || "HOLD",
              confidence:
                typeof rec === "string" ? "MEDIUM" : rec.confidence || "MEDIUM",
              score,
            },
            aiSummary,
            keyFactors:
              res.keyFactors || res.analysis?.insights?.keyStrengths || [],
            risks: res.risks || res.analysis?.insights?.riskFactors || [],
            nextCatalysts: res.nextCatalysts || [],
            currentPrice,
            priceContext:
              res.priceContext || res.analysis?.meta?.priceContext || "",
          });
        } else {
          setAnalysisResult({
            companyName: selectedCompany.name || selectedCompany.symbol,
            symbol: selectedCompany.symbol || "N/A",
            recommendation: { action: "HOLD", confidence: "MEDIUM", score: 55 },
            aiSummary: "Simulated analysis result.",
            keyFactors: ["Factor A", "Factor B"],
            risks: ["Risk 1"],
            nextCatalysts: [],
            currentPrice: "$123.45",
            priceContext: "+1.2%",
      ...
      toast({
        ...
        description: `Analyzed ${selectedCompany.symbol}`,
      });
    } catch (err: any) {
      ...
      <Card className="card-glass border-t-4 border-t-finance-primary">
        <CardHeader className="bg-gradient-to-r from-finance-primary/10 via-finance-accent/10 to-finance-secondary/10">
          ...
          <CardDescription className="text-muted-foreground">
            Get intelligent insights combining technical, fundamental, and
            sentiment analysis.
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          <div className="mb-6">
            ...
            <Tabs
              value={analysisMode}
              onValueChange={(v: any) => setAnalysisMode(v)}
              className="w-full"
            >
          ...
          <div className="mb-6">
            ...
            <Select
              ...
              onValueChange={(v: any) => setAnalysisDuration(v)}
            >
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-finance-primary to-finance-accent text-white text-lg py-6"
          >
            {isLoading ? (
          ...
          {analysisResult && (
            <div className="mt-8 space-y-8">
              ...
              <h2 className="text-responsive-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-finance-primary to-finance-accent text-center">
                Analysis Results for {analysisResult.companyName} (
              ...
              <Card className="card-glass border-l-4 border-l-finance-success">
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                    ...
                    {analysisResult.recommendation.action === "HOLD" && (
                      <Minus className="h-6 w-6 text-finance-warning" />
                    )}
                    {analysisResult.recommendation.action === "SELL" && (
                      <TrendingDown className="h-6 w-6 text-finance-danger" />
                    )}
                    ...
                    <Badge
                      variant="outline"
                      className={`text-lg font-semibold ${analysisResult.recommendation.confidence === "HIGH" ? "border-finance-success text-finance-success" : analysisResult.recommendation.confidence === "MEDIUM" ? "border-finance-warning text-finance-warning" : "border-finance-danger text-finance-danger"}`}
                    >
                      Confidence: {analysisResult.recommendation.confidence}
                    </Badge>
                  </CardTitle>
                  <CardDescription className="text-muted-foreground">
                    Overall Hybrid Score: {analysisResult.recommendation.score}
                    /100
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="w-full bg-slate-100 rounded h-3 overflow-hidden">
                    <div
                      style={{
                        width: `${analysisResult.recommendation.score}%`,
                      }}
              ...
              <div className="border border-border rounded-lg overflow-hidden shadow-sm bg-card/50 p-2">
                <h3 className="text-xl font-bold text-foreground mb-2 px-4 pt-2">
                  Live Chart
                </h3>
                <PriceChart />
