...
export default function EnhancedWeightsPanel({ initial, onChange }: Props) {
  ...
  const getWeightingStyle = () => {
    ...
    if (fundamental === max && max > 40)
      return {
        style: "Fundamental Value Investing",
        description:
          "Focus on company fundamentals, financial health, and intrinsic value",
        icon: TrendingUp,
        color: "text-blue-600",
      };
    if (technical === max && max > 40)
      return {
        style: "Technical Trading Signals",
        description:
          "Emphasis on price patterns, indicators, and market momentum",
        icon: BarChart,
        color: "text-purple-600",
      };
    if (sentiment === max && max > 40)
      return {
    ...
    return {
      ...
      description:
        "Equal consideration of fundamentals, technicals, and sentiment",
      icon: Info,
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-5">
              ...
              <div key={row.label} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div
                      className={`text-xs px-2 py-1 rounded ${row.bgColor} ${row.color}`}
                    >
                      {row.value}
                    </div>
                  </div>
          ...
          <div className="text-xs text-muted-foreground bg-accent/20 p-3 rounded-lg border">
            ...
            Weights are automatically normalized to 100% when analysis is
            performed.
          </div>
        </div>
