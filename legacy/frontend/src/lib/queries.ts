...
export function useSearch(query: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const { data } = await api.get("/api/stocks/search", {
          params: { query },
        });
...
export function useTrending(category: "gainers" | "losers" | "mostActive") {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get("/api/stocks/trending", {
        params: { category },
      });
...
export function useAnalysis(params: AnalysisParams) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(`/api/stocks/analysis/${symbol}`, {
        params: rest,
      });
