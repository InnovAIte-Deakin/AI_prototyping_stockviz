...
class SearchController {
    ...
    try {
      ...
      if (!query || query.length < 1) {
        return res.status(400).json({
          status: "error",
          message:
            "Query parameter is required and must be at least 1 character",
        });
      }
    ...
    for (const r of list.slice(0, 10)) {
      ...
      try {
        ...
        const exact = matches.find(
          (m) => m.symbol.toUpperCase() === r.symbol.toUpperCase(),
        );
  ...
  async getTrendingSymbols(req, res) {
    try {
      console.log(
        "📈 Fetching trending symbols via enhanced trending service...",
      );
      ...
      const trendingData =
        await enhancedTrendingService.getTrendingStocks(catKey);
