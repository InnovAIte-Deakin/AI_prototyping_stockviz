...
class StockController {
  ...
  async getTechnicalIndicators(req, res) {
    try {
      const availableIndicators =
        technicalAnalysisService.getAvailableIndicators();
  ...
  parseIndicatorsConfig(indicatorsParam) {
    ...
    try {
      ...
      console.warn(
        "Invalid indicators parameter, using default configuration:",
        error.message,
      );
      return {};
    }
  ...
  async getStockAnalysis(req, res) {
    try {
      ...
      console.log(
        `📊 Analyzing ${symbol.toUpperCase()} - ${timeframe} - ${mode} mode`,
      );
      if (mode === "advanced") {
        console.log(
          `⚖️  Weights: F:${weights.fundamental}% T:${weights.technical}% S:${weights.sentiment}%`,
        );
        console.log(
          `📈 Indicators: ${Object.keys(indicatorsConfig).join(", ") || "Default"}`,
        );
      ...
      let analysis;
      if (mode === "normal") {
        analysis = await analysisService.performNormalAnalysis(
          symbol,
          stockData,
          timeframe,
        );
  ...
  async getTechnicalChartAnalysis(req, res) {
    try {
      ...
      console.log(
        `📊 Starting technical chart analysis for ${symbol.toUpperCase()} - ${timeframe} - ${userMode} mode`,
      );
      console.log(
        `🔍 Chart Analysis: Candlestick patterns, volume analysis, RSI/DMI interpretation`,
      );
      console.log(
        `🎯 Focus: Support/resistance levels and trade setup recommendations`,
      );
      ...
      console.log(
        `✅ Technical chart analysis completed for ${symbol.toUpperCase()}`,
      );
      ...
      res.status(500).json({
        ...
        error: error.message,
        fallback:
          "Chart analysis is temporarily unavailable. Please try the regular analysis endpoint.",
      });
    }
  }
  ...
  async getEnhancedAnalysis(req, res) {
    try {
      ...
      console.log(
        `🚀 Enhanced analysis for ${symbol.toUpperCase()} - ${timeframe}`,
      );
      console.log(
        `⚖️  Enhanced Weights: F:${weights.fundamental}% T:${weights.technical}% S:${weights.sentiment}%`,
      );
      console.log(
        `📈 Advanced Scoring: Confidence levels, detailed breakdown, and reasoning`,
      );
      console.log(
        `🎯 Enhanced Algorithms: Integrated from stock-whisperer-frontend`,
      );
      ...
      console.log(
        `✅ Enhanced analysis completed for ${symbol.toUpperCase()}: ${enhancedAnalysis.analysis.enhanced.aggregateScore}/100 (${enhancedAnalysis.analysis.enhanced.recommendation})`,
      );
      ...
      res.status(500).json({
        ...
        error: error.message,
        fallback:
          "Enhanced analysis is temporarily unavailable. Please try the regular analysis endpoint.",
      });
    }
  }
}
