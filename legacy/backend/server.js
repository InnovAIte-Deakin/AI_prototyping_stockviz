...
app.get("/test-connections", async (req, res) => {
  ...
  res.json({
    ...
    overall_status:
      alphaVantageOK && geminiOK ? "ready" : "configuration_needed",
  });
});
...
app.get("/", (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      customWeightAnalysis:
        "/api/stocks/analysis/AAPL?timeframe=6M&mode=advanced&fundamental=30&technical=40&sentiment=30",
      customIndicators:
        '/api/stocks/analysis/AAPL?timeframe=1M&mode=advanced&indicators={"RSI":{"period":14},"MACD":{"fastPeriod":12,"slowPeriod":26,"signalPeriod":9}}',
      disablePatterns:
        '/api/stocks/analysis/AAPL?timeframe=1M&mode=advanced&indicators={"patterns":{"enabled":false}}',
    ...
    examples: {
      ...
      advancedAnalysis:
        'curl "http://localhost:3001/api/stocks/analysis/AAPL?timeframe=3M&mode=advanced"',
      customWeights:
        'curl "http://localhost:3001/api/stocks/analysis/AAPL?timeframe=3M&mode=advanced&fundamental=30&technical=40&sentiment=30"',
      customIndicators:
      ...
      disablePatterns:
        'curl "http://localhost:3001/api/stocks/analysis/AAPL?timeframe=1M&mode=advanced&indicators={\"patterns\":{\"enabled\":false}}"',
    },
    timeframeOptions: {
      description:
        "Timeframe parameter controls the historical data period for analysis",
      options: [
...
app.use("*", (req, res) => {
  res.status(404).json({
    ...
    message:
      "Include timeframe parameter for stock analysis (e.g., timeframe=1M)",
    available_endpoints: [
      "/",
      "/health",
      "/test-keys",
...
app.listen(PORT, async () => {
  ...
  console.log(
    `🗑️  Clear Cache: http://localhost:${PORT}/cache/clear (DELETE or GET)`,
  );
  console.log(
    `🧹 Cleanup Cache: http://localhost:${PORT}/cache/cleanup (POST or GET)`,
  );
  console.log(
    `📈 Technical Indicators: http://localhost:${PORT}/api/stocks/indicators`,
  );
  console.log(
    `🔎 Search Stocks: http://localhost:${PORT}/api/stocks/search?query=apple`,
  );
  console.log(`📈 Trending: http://localhost:${PORT}/api/stocks/trending`);
  console.log(
    `📊 Normal Analysis: http://localhost:${PORT}/api/stocks/analysis/AAPL?timeframe=1M&mode=normal`,
  );
  console.log(
    `⚖️  Advanced Analysis: http://localhost:${PORT}/api/stocks/analysis/AAPL?timeframe=3M&mode=advanced`,
  );
  console.log(
    `🎯 Custom Weights: http://localhost:${PORT}/api/stocks/analysis/AAPL?timeframe=3M&mode=advanced&fundamental=30&technical=40&sentiment=30`,
  );
  console.log(
    `⚙️  Custom Indicators: http://localhost:${PORT}/api/stocks/analysis/AAPL?timeframe=1M&mode=advanced&indicators={"RSI":{"period":14},"MACD":{"fastPeriod":12,"slowPeriod":26,"signalPeriod":9}}`,
  );
  console.log(
    `🚫 Disable Patterns: http://localhost:${PORT}/api/stocks/analysis/AAPL?timeframe=1M&mode=advanced&indicators={"patterns":{"enabled":false}}`,
  );
  console.log(`📊 API Tracking: http://localhost:${PORT}/api/tracking`);
  console.log(`📋 API Calls: http://localhost:${PORT}/api/tracking/calls`);
  console.log(
    `🗑️  Clear API Tracking: http://localhost:${PORT}/api/tracking/clear (DELETE)`,
  );
  console.log(
    `⚙️  Default Weights: http://localhost:${PORT}/api/stocks/weights/defaults`,
  );
  console.log("\n⏰ Timeframe Options:");
  ...
  if (alphaVantageOK && geminiOK) {
    ...
    console.log(
      "⚠️ Some API connections failed. Check /test-connections for details.\n",
    );
  }
});
