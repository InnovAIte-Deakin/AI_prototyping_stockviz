...
class GeminiService {
  ...
  getEnhancedFallbackResponse(prompt, userMode = "normal") {
    ...
    const symbolMatch = prompt.match(
      /(?:STOCK ANALYSIS FOR|ANALYSIS:|FOR) ([A-Z]{1,5})/,
    );
    const symbol = symbolMatch ? symbolMatch[1] : "the stock";

    const fundamentalScoreMatch = prompt.match(
      /(?:Health Score|Composite Score|Fundamental.*?):\s*(\d+)/,
    );
    const fundamentalScore = fundamentalScoreMatch
      ? parseInt(fundamentalScoreMatch[1])
      : 50;

    const technicalScoreMatch = prompt.match(
      /(?:Trend Strength|Technical Score):\s*(\d+)/,
    );
    const technicalScore = technicalScoreMatch
      ? parseInt(technicalScoreMatch[1])
      : 50;

    const sentimentScoreMatch = prompt.match(
      /(?:Investor Sentiment|Sentiment Score):\s*(\d+)/,
    );
    const sentimentScore = sentimentScoreMatch
      ? parseInt(sentimentScoreMatch[1])
      : 50;

    const overall = Math.round(
      (fundamentalScore + technicalScore + sentimentScore) / 3,
    );
    ...
    if (userMode === "advanced") {
      return this.getProFallbackSummary(
        symbol,
        overall,
        fundamentalScore,
        technicalScore,
      ...
      return this.getBeginnerFallbackSummary(
        symbol,
        overall,
        fundamentalScore,
        technicalScore,
...
**Remember:** Help the reader **learn technical analysis** while analyzing this specific chart!`
  ...
  formatTechnicalChartData(data) {
    if (!data || !data.indicators)
      return "Technical chart data not available for detailed analysis.";
  ...
  async generateTechnicalChartAnalysis(
    symbol,
    technicalData,
    userMode = "normal",
  ) {
    const prompt = this.buildTechnicalChartAnalysisPrompt(
      symbol,
      technicalData,
      userMode,
    );
  ...
  formatBeginnerFundamentals(data) {
    ...
    if (breakdown.valuation) {
      const valueStatus =
        breakdown.valuation >= 70
          ? "fairly priced"
          : breakdown.valuation >= 40
            ? "might be expensive"
    ...
    if (breakdown.profitability) {
      const profitStatus =
        breakdown.profitability >= 70
          ? "very profitable"
          : breakdown.profitability >= 40
...
*Educational analysis - Learn more about investing fundamentals*`;
  ...
  getEntryStrategy(overall, technicalScore) {
    if (overall >= 70 && technicalScore >= 60)
      return "Aggressive - Full position";
  ...
  getPositionSize(overall, confidence) {
    ...
    const confidenceMultiplier = confidence.includes("High")
      ? 1.5
...
- ${this.getBeginnerRSIExplanation(indicators.RSI.current)}`
    : '- We need RSI data to check if the stock is "tired" or "energetic"'
}
...
*Educational technical analysis - Practice reading charts to improve your skills!*`,
  ...
  getTrendDirection(indicators) {
    ...
    if (
      ...
      indicators.SMA.signal.includes("Above")
    )
      return "upward";
    if (
      indicators.SMA &&
      indicators.SMA.signal &&
      indicators.SMA.signal.includes("Below")
    )
      return "downward";
    return "sideways";
  }
  ...
  getBeginnerRSIExplanation(rsi) {
    if (rsi > 70)
      return '**Too Excited!** The stock might be "overheated" and could cool down soon.';
    if (rsi < 30)
      return '**Too Tired!** The stock might be "oversold" and could bounce back up.';
    if (rsi > 50)
      return "**Feeling Good!** The stock has positive energy and momentum.";
...
- **What to Watch:** If price breaks through resistance, it might keep going up!`;
  ...
  getSimpleTrendExplanation(indicators) {
    ...
    switch (trend) {
      case "bullish":
        return "**positive energy** - like a rocket trying to go up! 🚀";
      case "bearish":
        return "**negative pressure** - like a ball rolling downhill 📉";
      case "upward":
        return "**climbing stairs** - going up step by step 📈";
      case "downward":
        return "**sliding down** - losing altitude gradually 📉";
      default:
        return "**taking a rest** - moving sideways like a calm lake 🏞️";
    }
  }
  ...
  getTimeHorizon(indicators) {
    if (
      indicators.MACD &&
      indicators.MACD.signal &&
      indicators.MACD.signal.includes("Strong")
    )
      return "medium-term (2-8 weeks)";
    if (
      indicators.RSI &&
      indicators.RSI.current &&
      (indicators.RSI.current > 70 || indicators.RSI.current < 30)
    )
  ...
  calculateTechnicalConfidence(technicalData) {
    ...
    if (indicators.RSI) {
      total++;
      if (indicators.RSI.signal && !indicators.RSI.signal.includes("Neutral"))
        signals++;
    }

    if (indicators.MACD) {
      total++;
      if (indicators.MACD.signal && !indicators.MACD.signal.includes("Neutral"))
        signals++;
    }
    ...
    if (indicators.BollingerBands) {
      ...
      if (
        ...
        !indicators.BollingerBands.signal.includes("Neutral")
      )
        signals++;
    }
