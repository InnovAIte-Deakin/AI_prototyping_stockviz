...
class DataService {
  ...
  async fetchStockData(symbol, timeframe) {
    console.log(
      `📊 Fetching data for ${symbol} (${timeframe}) via DataSourceManager`,
    );
  ...
  async fetchAlphaVantageData(symbol, timeframe) {
    ...
    try {
      ...
      let endpoint;
      if (
        ["1min", "5min", "15min", "30min", "60min", "5min", "30min"].includes(
          interval,
        )
      ) {
      ...
      if (!timeSeries || Object.keys(timeSeries).length === 0) {
        console.warn(
          "Alpha Vantage returned no time series data (keys: " +
