...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error(
    "[cleanup] Missing SUPABASE_SERVICE_ROLE_KEY or NEXT_PUBLIC_SUPABASE_URL environment variable",
  );
  process.exit(1);
}
...
async function cleanupExpiredCache(batchSize: number = 1000): Promise<number> {
  ...
  while (batchNum < maxBatches) {
    ...
    console.log(
      `[cleanup] Batch ${batchNum + 1}: deleted ${deletedCount} entries`,
    );
...
async function main() {
  ...
  console.log(
    `[cleanup] Cache cleanup complete: ${cacheDeleted} entries deleted`,
  );

  const apiDeleted = await cleanupOldApiLogs();
  console.log(
    `[cleanup] API log cleanup complete: ${apiDeleted} entries deleted`,
  );
