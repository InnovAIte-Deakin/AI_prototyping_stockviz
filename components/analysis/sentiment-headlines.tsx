...
export function SentimentHeadlines({
  sentiment,
  symbol,
}: SentimentHeadlinesProps) {
  const headlines = normalizeHeadlines(sentiment);
  const source = sentiment?.source || "Sentiment feed";
  const summary =
    sentiment?.summary ||
    `No detailed sentiment summary is available for ${symbol} right now.`;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[#6a706f]">
              {source} • {headlines.length} recent article
              {headlines.length === 1 ? "" : "s"}
            </p>
          </div>
        </div>
      ...
      {headlines.length > 0 ? (
        ...
        <div className="mt-6 rounded-[20px] border border-dashed border-[#ded7d2] bg-[#fbf8f6] p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-[#6a706f]">
            The analysis service returned a neutral sentiment fallback for{" "}
            {symbol}. Once a news source responds, recent article summaries will
            appear here.
          </p>
        </div>
      )}
