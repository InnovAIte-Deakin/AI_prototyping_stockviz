...
class SentimentService {
  ...
  async fetchAlphaVantageNewsSentiment(symbol) {
    const apiKey = getAlphaVantageApiKey(this.env);
    if (!apiKey)
      throw new Error("ALPHA_VANTAGE_API_KEY or MARKET_DATA_API_KEY missing");
    ...
    try {
      ...
      const average =
        feed.reduce(
          (total, item) => total + (Number(item.overall_sentiment_score) || 0),
          0,
        ) / feed.length;
