...
function getAlphaVantageApiKey(env = process.env) {
  return env.ALPHA_VANTAGE_API_KEY || env.MARKET_DATA_API_KEY;
}
...
class FundamentalAnalysisService {
  constructor({
    ...
    this.env = env;
    this.logger = logger;
    this.key = getAlphaVantageApiKey(env);
    this.weights = {
      valuation: 25,
      growth: 20,
      profitability: 25,
  ...
  async analyze(symbol) {
    ...
    try {
      if (this.dataSourceManager?.fetchFundamentalData) {
        try {
          const fundamentalData =
            await this.dataSourceManager.fetchFundamentalData(normalizedSymbol);
          if (fundamentalData?.overview) {
            const metrics = this.buildMetricsFromMultiSource(
          ...
          this.logger.warn(
            "Multi-source fetch failed, falling back to Alpha Vantage:",
            error.message,
          );
        }
      }
      ...
      const result = {
        ...
        metrics,
        notes:
          "Computed from Alpha Vantage fundamentals (OVERVIEW/INCOME/BALANCE/CASH).",
      ...
      this.logger.warn(
        "Fundamentals pipeline failed, using neutral fallback:",
        error.message,
      );
      return {
        score: 50,
  ...
  async fetchAlphaVantageJson(functionName, symbol) {
    if (!this.key)
      throw new Error("Missing ALPHA_VANTAGE_API_KEY or MARKET_DATA_API_KEY");
  ...
  buildMetrics(overview, incomeAnnual, balanceAnnual, cashAnnual) {
    ...
    const profitMargin =
      pm ??
      (safeDiv(lastIncome.netIncome, lastIncome.totalRevenue) ?? null) * 100;
    const operatingMargin =
      opm ??
      (safeDiv(lastIncome.operatingIncome, lastIncome.totalRevenue) ?? null) *
        100;
    const returnOnEquity =
      roe ??
      (safeDiv(lastIncome.netIncome, lastBalance.totalShareholderEquity) ??
        null) * 100;

    const debtToEquity = safeDiv(
      lastBalance.totalLiabilities,
      lastBalance.totalShareholderEquity,
    );
    const currentRatio = safeDiv(
      lastBalance.totalCurrentAssets,
      lastBalance.totalCurrentLiabilities,
    );
    const fcfLast =
      lastCash.operatingCashflow === null ||
      lastCash.operatingCashflow === undefined
        ? null
        : lastCash.operatingCashflow - (lastCash.capitalExpenditures ?? null);
    const fcfMargin =
      fcfLast !== null && lastIncome.totalRevenue
        ? (fcfLast / lastIncome.totalRevenue) * 100
        : null;
    const interestCoverage =
      lastIncome.interestExpense !== null && lastIncome.interestExpense !== 0
        ? safeDiv(
            Math.abs(lastIncome.operatingIncome ?? 0),
            Math.abs(lastIncome.interestExpense),
          )
        : null;
  ...
  scoreCategories(metrics) {
    ...
    const growth = Math.round(
      nanOr(higherBetter(metrics.revenueCAGR, 0, 15), 50),
    );
