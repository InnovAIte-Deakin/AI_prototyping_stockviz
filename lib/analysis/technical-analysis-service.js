import TechnicalIndicators from "@thuantan2060/technicalindicators";
...
class TechnicalAnalysisService {
  ...
  async calculateIndicators(ohlcvData, indicatorsConfig = {}) {
    ...
    try {
      for (const [indicator, settings] of Object.entries(config)) {
        ...
        this.applyIndicator(results, indicator, settings, {
          closes,
          highs,
          lows,
          volumes,
  ...
  getCustomIndicators(ohlcvData, indicatorsConfig = {}) {
    ...
    try {
      ...
      if (
        indicatorsConfig.patterns === undefined ||
        indicatorsConfig.patterns.enabled !== false
      ) {
        results.patterns = this.recognizePatterns(ohlcvData);
      }
  ...
  applyIndicator(results, indicator, settings, data) {
    switch (indicator) {
      ...
      case "RSI":
        ...
        break;
      case "Stochastic":
        results.Stochastic = this.calculateStochastic(
          data.highs,
          data.lows,
          data.closes,
          settings,
      ...
      case "CCI":
        ...
        break;
      case "WilliamsR":
        results.WilliamsR = this.calculateWilliamsR(
          data.highs,
          data.lows,
          data.closes,
          settings.period,
      ...
      case "StandardDeviation":
        results.StandardDeviation = this.calculateStandardDeviation(
          data.closes,
          settings.period,
        );
        break;
      case "VolumeSMA":
        results.VolumeSMA = this.calculateVolumeSMA(
          data.volumes,
          settings.period,
        );
        break;
      case "MoneyFlowIndex":
        ...
        break;
      case "Ichimoku":
        results.Ichimoku = this.calculateIchimokuCloud(
          data.highs,
          data.lows,
          data.closes,
          settings,
...
const technicalAnalysisServiceModule = {
  TechnicalAnalysisService,
  createTechnicalAnalysisService,
};

export { TechnicalAnalysisService, createTechnicalAnalysisService };

export default technicalAnalysisServiceModule;
