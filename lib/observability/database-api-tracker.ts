  ...
  async getAPIStatistics(): Promise<ApiStats> {
    try {
      ...
      const { data: recentCalls } = await supabase
        .from("api_call_log")
        .select(
          "id, api_name, endpoint, symbol, timeframe, success, response_time, created_at",
        )
      ...
      if (aggregatedStats && Array.isArray(aggregatedStats)) {
        ...
        if (allCalls) {
          ...
          for (const bucket of Object.values(stats.apis)) {
            if (bucket.total > 0) {
              bucket.averageResponseTime = Math.round(
                bucket.averageResponseTime / bucket.total,
              );
            }
          }
  ...
  async getStatsByApiName(apiName: string): Promise<{
    ...
    try {
      ...
      stats.averageResponseTime = Math.round(
        stats.averageResponseTime / stats.total,
      );
      return stats;
    } catch (error) {
...
export function createDatabaseApiTracker(options?: {
  maxRecords?: number;
  logger?: Console;
}) {
