...
| Story ID | Story | Priority | Owner | Status | Sprint | Checkpoint |
| --- | --- | --- | --- | --- | --- | --- |
| E3-S1 | Extract market data provider layer | `P0` | `Backend` | `not_started` | Sprint 2 | Stock data functions run from root `lib/market/` |
| E3-S2 | Extract analysis engine | `P0` | `Backend` | `done` | Sprint 2 | Fundamental, technical, sentiment, and scoring run without Express |
| E3-S3 | Extract AI summary service | `P1` | `Backend` | `in_progress` | Sprint 2 | AI generation is isolated behind one interface |
| E3-S4 | Replace cache strategy | `P1` | `Backend` | `not_started` | Sprint 2 | Caching works without process memory assumptions |
| E3-S5 | Replace API tracking or remove it | `P2` | `Backend` | `not_started` | Sprint 2 | Tracking no longer depends on in-memory Express state |
...
| Story ID | Story | Priority | Owner | Status | Sprint | Checkpoint |
| --- | --- | --- | --- | --- | --- | --- |
| E5-S1 | Build symbol search service and endpoint | `P1` | `Fullstack` | `not_started` | Sprint 3 | Search data is available from root app |
| E5-S2 | Migrate search UI | `P1` | `Frontend` | `not_started` | Sprint 3 | User can search and navigate to a symbol |
| E5-S3 | Add `/analysis/[symbol]` route | `P1` | `Fullstack` | `not_started` | Sprint 3 | Symbol analysis page exists |

### E6: Core Analysis Experience

| Story ID | Story | Priority | Owner | Status | Sprint | Checkpoint |
| --- | --- | --- | --- | --- | --- | --- |
| E6-S1 | Migrate analysis page layout | `P1` | `Frontend` | `not_started` | Sprint 3 | Core analysis screen renders in root app |
| E6-S2 | Migrate chart and summary panels | `P1` | `Frontend` | `not_started` | Sprint 3 | Analysis page shows actionable insight |
| E6-S3 | Migrate recommendation and scoring UI | `P1` | `Frontend` | `not_started` | Sprint 3 | Score and recommendation are visible |
...
- cache and API tracking redesign are still pending
...
| 3 | Extract `dataService.js` and `dataSourceManager.js` into root `lib/market/` | `Backend` | `not_started` |
...
| 6 | Isolate Gemini, cache, and API tracking behind root adapters | `Backend` | `not_started` |
| 7 | Build symbol search and `/analysis/[symbol]` route with live data | `Fullstack` | `not_started` |
| 8 | Migrate score, recommendation, summary, and chart components | `Frontend` | `not_started` |
...
- `dataService.js` and `dataSourceManager.js` are still legacy-owned
- `geminiService.js` is still injected from the legacy backend layer
- `cacheService.js` remains in-memory and legacy-owned (Supabase `analysis_cache` table ready as replacement)
- `apiTrackingService.js` remains in-memory and legacy-owned (Supabase `api_call_log` table ready as replacement)
- no Next route handlers use the extracted analysis modules yet
